...
{
    public GameObject nowCharacter; // 지금 캐릭터
    ...
    {
        (string statusValueKey, float[] status) = nowCharacter.GetComponent<Character>().GetStatus(); // statusKey는 전부 통일로 변경
    ...
    public void SetNoUpgrades() // (테스트룸 전용) 업그레이드를 안 했을 때 실행
    {
    ...
    private void SetNowSelectIdxX(int newIdxX) // X값만 변경
    {
        ...
        else if (nowCharacterSelectIdxY == 2 && nowCharacterSelectIdxX == -1 && nowCharacterSelectIdxX > newIdxX) // 가장 오른쪽 아래 보유 스킬에서 오른쪽을 누르면 업그레이드 버튼으로
        {
            nowCharacterSelectIdxY = 1;
            nowCharacterSelectIdxX = -1;
        }
